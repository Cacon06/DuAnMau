...
public class Sach_DAO {
    ...
    public List<Sach_DTO> getxapxep(String dieukien) {
        String sql = ("Select * From Sach  order By GiaThue "+dieukien);
        return getdata(sql);
    }
    public List<Sach_DTO> getimKiem(String name) {
    ...
    public List<Sach_DTO> getdata(String sql, String... dieukien) {
        ...
        if (c != null && c.getCount() > 0) {
            c.moveToFirst();
            do {
                list.add(new Sach_DTO(c.getInt(0), c.getString(1), c.getInt(2),c.getInt(3),c.getInt(4)));
            } while (c.moveToNext());
        }
